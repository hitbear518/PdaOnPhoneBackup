...
unsigned char NUM2HEX(unsigned char x) 
{
	return (x>9?x+'a'-10:x+'9'-9);
}

unsigned char HEX2NUM(unsigned char c) 
{
	return (c>'9'?c+10-'a':c+9-'9');
}

void MD5toString(unsigned char data[16], char *str) 
{
	for(int i=0; i<16; i++)
	{
...
void MD5String(const unsigned char *buf, unsigned int len, char *str)
{
